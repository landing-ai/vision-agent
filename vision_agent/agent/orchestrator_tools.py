...
def generate_vision_code(save_file: str, chat: str, media: List[str]) -> str:
    ...
    try:
        ...
        return f"[File {save_file}]\n" + response["code"] + "\n[End of file]"
    except Exception as e:
        return str(e)


def edit_vision_code(code_file: str, chat_history: List[Message]) -> str:
    agent = va.agent.VisionAgent()
    try:
        response = agent.chat_with_workflow(chat_history)
        with open(Path(WORKSPACE) / code_file, "w") as f:
            f.write(response["code"])
        return f"[File {code_file}]\n" + response["code"] + "\n[End of file]"
    except Exception as e:
        return str(e)
